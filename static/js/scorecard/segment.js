...
function init() {
  ...
  showScatterplot({
      ...
      y:     {field: "attendance.academic_year", label: "Cost Attendance"}, 
      size:  {field: "enrollment.all", label: "Enrollment"}, 
      color: {field: "ownership", label: "Control", 
  ...
  showScatterplot({
    ...
    symbol: {field: "ownership", label: "Control",
    ...
    color: {field: "6_yrs_after_entry.working_not_enrolled.mean_earnings", label: "Earnings 2 yrs after grad"},
    filterFunction: function(rec) {
      return rec["6_yrs_after_entry.working_not_enrolled.mean_earnings"] 
        && rec["median_debt_suppressed.overall"]
        && rec["demographics.median_hh_income"] 
        && rec["carnegie_undergrad"] 
        && +rec["carnegie_undergrad"] > 5
        && rec["enrollment.all"] 
        && +rec["enrollment.all"] > 1000
    },
    colorScale: "quantize",

    rank:       {field: "usnews_2019_rank", label: "US News Rank"},
    carnegieug: {field: "carnegie_undergrad", label: "Carnegie Categories (undergrad)"},
    enrollment: {field: "enrollment.all", label: "Enrollment"}
  });
}
